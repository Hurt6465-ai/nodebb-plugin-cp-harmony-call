(function () {
  ...
  var CLOSED_CALLS_KEY = "cp_harmony_closed_calls_v4";
  var RING_UNLOCKED_KEY = "__cpHarmonyCallAudioUnlockedV4";
  ...
  async function fetchBridgeToken() {
    try {
      var res = await fetch(withRelativePath(UserConfig.tokenPath || "/bridge/token"), { credentials: "same-origin" });
      if (!res.ok) return {};
  ...
  async function ensureWukong() {
    ...
    State.wkReadyPromise = (async function () {
      ...
      if (!window.__cpHarmonyCallWkListenerBoundV4) {
        window.__cpHarmonyCallWkListenerBoundV4 = true;
  ...
  function getPeerUid() {
    ...
    if (window.ajaxify && window.ajaxify.data && Array.isArray(window.ajaxify.data.users)) {
      for (var i = 0; i < window.ajaxify.data.users.length; i++) {
        ...
        if (String(u.uid) !== me && String(u.uid) !== "0") {
          return String(u.uid);
        }
      }
    }
    ...
    var row = document.querySelector(
      '[component="chat/user/list"] [data-uid]:not([data-uid="' + me + '"])'
    );
  ...
  function injectStyle() {
    ...
    var css = `
      ...
      .cp-harmony-call-entry{width:34px;height:34px;min-width:34px;padding:0;border:none;border-radius:999px;background:transparent;color:#111827;display:inline-flex;align-items:center;justify-content:center;cursor:pointer;box-shadow:none;-webkit-tap-highlight-color:transparent;}
      .cp-harmony-call-entry:hover{opacity:.72;}
      .cp-harmony-call-entry:active{transform:scale(.92);}
      .cp-harmony-call-entry i{font-size:17px;line-height:1;}
      ...
      #cp-call-avatar{width:104px;height:104px;border-radius:40%;object-fit:cover;border:1px solid rgba(255,255,255,.18);box-shadow:0 16px 40px rgba(0,0,0,.32);background:#334155;}
      ...
      #cp-call-in-avatar{width:112px;height:112px;border-radius:40%;object-fit:cover;background:#334155;border:1px solid rgba(255,255,255,.14);box-shadow:0 16px 40px rgba(0,0,0,.34);}
  ...
  function injectHeaderButton() {
    ...
    if (!window.__cpHarmonyCallPopOutsideBoundV4) {
      window.__cpHarmonyCallPopOutsideBoundV4 = true;
  ...
  function boot() {
    ...
    if (!window.__cpHarmonyCallBeforeUnloadBoundV4) {
      window.__cpHarmonyCallBeforeUnloadBoundV4 = true;
    ...
    if (window.jQuery && !window.__cpHarmonyCallJqueryEventsBoundV4) {
      window.__cpHarmonyCallJqueryEventsBoundV4 = true;

      window.jQuery(window).on("action:ajaxify.end action:chat.loaded action:chat.switched", function () {
        setTimeout(injectHeaderButton, 80);
        setTimeout(hideSignalMessagesInDom, 180);
      });
    }
  }
